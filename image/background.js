...
function createBackground(url, colorObject, callback) {
  ...
  fs.exists(path, function(exists) {
    ...
    utils.download(url, cacheFile, function() {
      ...
      var command = "convert " + cacheFile + " -colorspace RGB \\( -clone 0 -fill " + rgb + " -colorize 80% -auto-level \\) \\( -clone 0 -colorspace gray -auto-level \\) -compose blend -define compose:args=80x20 -composite -sigmoidal-contrast 20,60% +dither -colors 10 -auto-level -modulate 150,150 -resize 480x270\^ -gravity center -crop 480x270+0+0 -sharpen 3x3 -blur 5,1 " + path;
      console.log(command);
